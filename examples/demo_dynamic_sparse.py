...
def demo_dynamic_allocation():
    ...
    for name, config in layer_configs.items():
        ...
        total_params += size
        print(f"  - {name:6s}: {str(config['size']):20s} = {size:>6d} params "
              f"(importance: {config['importance']:.1f})")
    print(f"\n  Total: {total_params:,} parameters\n")
    
    # Initialize model
    model_weights = {
        name: np.random.randn(*config["size"]) * 0.1
        for name, config in layer_configs.items()
    }
    ...
    for name in sorted(layer_sparsities.keys(), key=lambda x: layer_sparsities[x]):
        ...
        print(f"  {name:6s}: {sparsity*100:>5.1f}% sparse → "
    ...
    total_pruned = sum(
        layer_sparsities[name] * weights.size
...
def demo_combined_rigl_dynamic():
    ...
    for name in sorted(model_weights.keys()):
        ...
        print(f"  {name}: {active:>4d}/{size:>4d} active "
...
def demo_comparison():
    ...
    print(f"  ✓ Loss: {losses_dense[-1]:.6f} | Time: {time_dense:.3f}s | "
          f"Params: {model_size}\n")
    ...
    print(f"  ✓ Loss: {losses_static[-1]:.6f} | Time: {time_static:.3f}s | "
    ...
    print(f"  ✓ Loss: {losses_rigl[-1]:.6f} | Time: {time_rigl:.3f}s | "
    ...
    for method, res in results.items():
        ...
        print(f"{method:<15} {res['final_loss']:<12.6f} {res['time']:<12.3f} "
