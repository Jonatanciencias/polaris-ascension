...
def demo_basic_integration():
    ...
    integration = create_integrated_search(
        num_classes=10, target_compression=2.0, max_memory_mb=4000.0  # Conservative for RX 580
    )
...
def demo_pareto_analysis(pareto_optimal, integration):
    ...
    print("-" * 80)
    print(
        f"{'Arch':<6} {'Acc':<8} {'Latency':<10} {'Memory':<10} {'Params':<10} {'Compression':<12}"
    )
    print("-" * 80)
...
def demo_preference_selection(pareto_optimal, integration):
    ...
    for preference in preferences:
        ...
        best_model, best_metrics = integration.mo_optimizer.select_best_architecture(
            pareto_optimal, preference=preference
        )
...
def demo_compression_comparison(integration, train_loader, val_loader):
    ...
    methods = [DecompositionMethod.TUCKER, DecompositionMethod.CP, DecompositionMethod.TENSOR_TRAIN]
    ...
    for method in methods:
        ...
        results[method.value] = {"compression": actual_compression, "time": decomp_time}
...
def demo_hardware_aware_search():
    ...
    search_config = DARTSConfig(num_cells=6, num_nodes=4, init_channels=16, layers=14)
