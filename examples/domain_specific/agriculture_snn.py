...
class CropHealthClassifier(nn.Module):
    ...
    def reset_state(self, batch_size: int):
        ...
        self.output_accumulator = torch.zeros(
    ...
    def get_statistics(self) -> Dict[str, Dict]:
        ...
        stats = {
            'layer1': self.snn1.get_all_statistics(),
            'layer2': self.snn2.get_all_statistics()
        }
        
        # Estimate energy efficiency
        total_spikes = (
            stats['layer1']['layer']['total_spikes'] +
            stats['layer2']['layer']['total_spikes']
        )
        max_possible_spikes = (
            self.n_timesteps * (self.n_hidden + self.n_classes)
        )
...
class IrrigationController(nn.Module):
    ...
    def reset_state(self, batch_size: int = 1):
        ...
        self.accumulated_evidence = torch.zeros(
            batch_size, self.n_actions, device=self.device
        )
