...
def benchmark_precision_optimization():
    ...
    print(f"  FP16: SNR = {fp16_results['snr_db']:.2f} dB {'✅' if fp16_results['screening_quality'] else '❌'}")
    print(f"  INT8: SNR = {int8_results['snr_db']:.2f} dB {'✅' if int8_results['screening_quality'] else '❌'}")
...
def benchmark_quantization_safety():
    ...
    medical_predictions = np.random.rand(1000, 10).astype(np.float32)  # Simulated class probabilities
    medical_result = analyzer.test_medical_safety(
        predictions=medical_predictions,
        bits=8,
        task='classification'
    )
...
def generate_comparison_table(results: Dict):
    ...
    for name, config in configs:
        ...
        print("{:<25} {:>12.2f} {:>15.0f} {:>15.2f} {:>10.2f}x".format(
            name,
            config['time_ms'],
            config['throughput'],
            config['memory_mb'],
