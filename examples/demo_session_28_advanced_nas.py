...
def demo_multi_branch():
    ...
    op = MultiBranchOperation(C=64, stride=1, branch_types=branch_types, use_gating=True)
    ...
    op_no_gate = MultiBranchOperation(C=64, stride=1, branch_types=branch_types, use_gating=False)
...
def demo_progressive_refinement():
    ...
    print(
        f"Coarse stage:  {progressive_config.coarse_epochs} epochs, "
        f"keep {progressive_config.coarse_keep_ratio*100:.0f}%"
    )
    print(
        f"Medium stage:  {progressive_config.medium_epochs} epochs, "
...
def demo_mixed_precision():
    ...
    config = MixedPrecisionConfig(
        available_precisions=[PrecisionLevel.FP32, PrecisionLevel.FP16, PrecisionLevel.INT8],
    ...
    for layer_name, precision in precision_map.items():
        ...
        print(
            f"{layer_name:12s}: {precision.value:5s} "
    ...
    total_bits_mixed = sum(precision.bits for precision in precision_map.values())
...
def demo_combined():
    ...
    branch_config = MultiBranchConfig(
        max_branches=3, branch_types=["conv", "attention", "identity"], use_gating=True
    )
    ...
    # Count parameters
    fp32_params = sum(
        sum(p.numel() for n, p in model.named_parameters() if layer_name in n)
...
def demo_hardware_specific():
    ...
    config = MixedPrecisionConfig(
        available_precisions=[PrecisionLevel.FP32, PrecisionLevel.INT8],  # Skip FP16, no benefit
