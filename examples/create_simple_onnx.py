...
try:
    ...
    W = helper.make_tensor(
        "W", TensorProto.FLOAT, [10, 5], np.random.randn(10, 5).flatten().tolist()
    )

    b = helper.make_tensor("b", TensorProto.FLOAT, [5], np.random.randn(5).flatten().tolist())

    # Create nodes
    # MatMul node: temp = input * W
    matmul_node = helper.make_node("MatMul", inputs=["input", "W"], outputs=["temp"])

    # Add node: output = temp + b
    add_node = helper.make_node("Add", inputs=["temp", "b"], outputs=["output"])

    # Create graph
    graph_def = helper.make_graph([matmul_node, add_node], "simple_linear", [X], [Y], [W, b])
