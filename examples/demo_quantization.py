...
def demo_per_channel_quantization():
    ...
    dequant_channel = quantizer.dequantize_tensor_per_channel(
        q_channel, scales_c, zps_c, axis=0
    )
...
def demo_mixed_precision():
    ...
    for name, weights in model_layers.items():
        ...
        print(f"{name:<10} {str(weights.shape):<20} {stats.sensitivity_score:<12.4f} "
