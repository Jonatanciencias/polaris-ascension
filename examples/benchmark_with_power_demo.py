...
def demo2_inference_with_power():
    ...
    try:
        ...
        results_cpu = profiler.profile_model(
            model_cpu,
            data_loader,
            duration=20,
            warmup_seconds=3
        ...
        if torch.cuda.is_available():
            ...
            print(f"  Power:        CPU: {results_cpu.avg_power_watts:.1f}W  |  GPU: {results_gpu.avg_power_watts:.1f}W")
            print(f"  FPS/Watt:     CPU: {results_cpu.fps_per_watt:.2f}  |  GPU: {results_gpu.fps_per_watt:.2f}")
...
def demo4_compare_batch_sizes():
    ...
    try:
        ...
        for batch_size in batch_sizes:
            ...
            print(f"  FPS: {result.fps:.1f}  |  Power: {result.avg_power_watts:.1f}W  |  FPS/W: {result.fps_per_watt:.2f}")
        ...
        for bs, res in results.items():
            print(f"{bs:>6} | {res.fps:>8.1f} | {res.avg_power_watts:>7.1f}W | "
