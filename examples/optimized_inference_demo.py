...
def demo_single_inference():
    ...
    for precision, name, emoji in modes:
        ...
        print(f"{emoji} {name:<22} {stats['mean']:>6.1f}ms{'':<7} "
...
def demo_batch_processing():
    ...
    for batch_size in batch_sizes:
        ...
        print(f"{batch_size:<15} {r['throughput']:.1f} imgs/sec{'':<8} "
              f"{r['avg_time']:.1f}ms{'':<14} {speedup:.2f}x")
    
    print("-" * 70)
    print(f"\n💡 KEY TAKEAWAY:")
    print(f"   • Batch processing improves GPU utilization")
    print(f"   • Batch size {max(results, key=lambda k: results[k]['throughput'])} "
