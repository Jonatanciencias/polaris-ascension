...
def main():
    parser = argparse.ArgumentParser(
        description="OpenCL GEMM with Power Monitoring"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run CPU baseline benchmark"
    )
    parser.add_argument(
        "--no-opencl", action="store_true",
        help="Skip OpenCL benchmark"
    )
    ...
    if not args.no_opencl:
        ...
        try:
            ...
            # Run benchmark
            opencl_result = run_power_benchmark(
                ctx, args.size, args.duration, "OpenCL GEMM"
            )
