...
class AIDemonstrator:
    ...
    def demonstrate_workgroup_prediction(self):
        ...
        test_configs = [
            {
                ...
                'wg_size_0': 4, 'wg_size_1': 64, 'wg_total_size': 256,
                'compute_units': 36, 'wavefront_size': 64, 'max_wg_size': 256
            },
            {
                'name': 'Aggressive Config',
                'wg_size_0': 8, 'wg_size_1': 32, 'wg_total_size': 256,
                'compute_units': 36, 'wavefront_size': 64, 'max_wg_size': 256
            },
            {
                'name': 'Balanced Config',
                'wg_size_0': 4, 'wg_size_1': 128, 'wg_total_size': 512,
                'compute_units': 36, 'wavefront_size': 64, 'max_wg_size': 256
            }
        ]
        ...
        for config in test_configs:
            print(f"\n🔍 Testing: {config['name']}")
            print(f"   Work-group: ({config['wg_size_0']}, {config['wg_size_1']}) = {config['wg_total_size']} threads")
    ...
    def demonstrate_memory_prediction(self):
        ...
        test_configs = [
            {
                ...
                'use_lds': 0, 'lds_tile_size': 0, 'vector_width': 1,
                'unroll_factor': 1, 'prefetch_distance': 0,
                'local_mem_size_kb': 64, 'global_mem_size_gb': 8.0
            },
            {
                'name': 'LDS Optimized',
                'use_lds': 1, 'lds_tile_size': 32, 'vector_width': 4,
                'unroll_factor': 4, 'prefetch_distance': 2,
                'local_mem_size_kb': 64, 'global_mem_size_gb': 8.0
            },
            {
                'name': 'Vectorized Memory',
                'use_lds': 0, 'lds_tile_size': 0, 'vector_width': 8,
                'unroll_factor': 8, 'prefetch_distance': 4,
                'local_mem_size_kb': 64, 'global_mem_size_gb': 8.0
            }
        ]
        ...
        for config in test_configs:
            print(f"\n🔍 Testing: {config['name']}")
            print(f"   LDS: {bool(config['use_lds'])}, Vector Width: {config['vector_width']}, Unroll: {config['unroll_factor']}")
    ...
    def demonstrate_combined_prediction(self):
        ...
        test_configs = [
            {
                ...
                'wg_size_0': 4, 'wg_size_1': 64, 'wg_total_size': 256,
                'wg_occupancy': 0.9, 'wg_efficiency': 0.95,
                'use_lds': 1, 'lds_tile_size': 32, 'vector_width': 4,
                'unroll_factor': 4, 'prefetch_distance': 2,
                'compute_units': 36, 'wavefront_size': 64,
                'local_mem_size_kb': 64, 'global_mem_size_gb': 8.0
            },
            {
                'name': 'Balanced Performance Config',
                'wg_size_0': 8, 'wg_size_1': 32, 'wg_total_size': 256,
                'wg_occupancy': 0.8, 'wg_efficiency': 0.85,
                'use_lds': 1, 'lds_tile_size': 16, 'vector_width': 2,
                'unroll_factor': 2, 'prefetch_distance': 1,
                'compute_units': 36, 'wavefront_size': 64,
                'local_mem_size_kb': 64, 'global_mem_size_gb': 8.0
            }
        ]
        ...
        for config in test_configs:
            print(f"\n🔍 Testing: {config['name']}")
            print(f"   Work-group: ({config['wg_size_0']}, {config['wg_size_1']}) | LDS: {bool(config['use_lds'])} | Vector: {config['vector_width']}")
    ...
    def show_system_stats(self):
        ...
        print(f"📈 Total Predictions Made: {stats['prediction_stats']['workgroup']['predictions'] + stats['prediction_stats']['memory']['predictions'] + stats['prediction_stats']['combined']['predictions']}")
        print(f"🔧 Libraries Available: XGBoost {stats['available_libraries']['xgboost']}, TensorFlow {stats['available_libraries']['tensorflow']}")
