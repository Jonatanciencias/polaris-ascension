...
def demo_structured_pruning():
    ...
    for metric in ["l1", "l2"]:
        ...
        print(f"{metric.upper():17s} | {len(kept_indices):13} | {pruned_weights.size:11,} | {reduction:8.1%}")
    ...
    print(f"  Memory:         {pruned_weights.nbytes:,} bytes ({pruned_weights.nbytes / weights.nbytes:.1%})")
...
def demo_gradual_pruning():
    ...
    for step in [1000, 3000, 5000, 7000, 9000, 10000]:
        ...
        print(f"  Step {step:5,}: {actual_sparsity:5.1%} sparse ({np.count_nonzero(mask):,} params kept)")
...
def demo_sparse_matmul_benchmark():
    ...
    for size in sizes:
        for sparsity in sparsities:
            ...
            print(f"{size[0]}x{size[1]:4} | {sparsity:7.0%} | {dense_time*1000:9.3f}ms | {sparse_time*1000:10.3f}ms | {theoretical_speedup:6.2f}x*")
    ...
    sparse_bytes = (len(csr['values']) * 8 +  # values
                    len(csr['col_indices']) * 4 +  # int32 indices
                    len(csr['row_pointers']) * 4)   # int32 pointers
...
def demo_memory_reduction():
    ...
    for sparsity in [0.5, 0.7, 0.9, 0.95]:
        ...
        for name in layers:
            ...
            sparse_memory += (nonzero * 8 +  # values (float64)
                            nonzero * 4 +  # col_indices (int32)
                            layers[name].shape[0] * 4)  # row_pointers (int32)
        
        compression = total_memory / sparse_memory if sparse_memory > 0 else 0
        
        print(f"{sparsity:7.0%} | {stats['remaining_parameters']:11,} | {sparse_memory:11,} | {compression:10.2f}x")
