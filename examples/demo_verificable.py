    ...
    for name, url in DEMO_IMAGES.items():
        ...
        try:
            ...
            req = urllib.request.Request(
                url, headers={"User-Agent": "Mozilla/5.0 (RX580-AI-Framework)"}
            )
...
def classify_images(
    image_paths: List[Path], model_path: Path, labels: List[str], benchmark: bool = False
    ...
    for img_path in image_paths:
        ...
        try:
            ...
            top_label = (
                labels[top_pred["class_id"]]
                if top_pred["class_id"] < len(labels)
                else f"class_{top_pred['class_id']}"
            )
...
def print_summary(results: List[dict], times: List[float], demo_dir: Path):
    ...
    for r in results:
        print(
            f"   • {r['image']:15s} → {r['prediction']:30s} "
...
Ejemplos:
  ...
  python examples/demo_verificable.py --images-dir /ruta/a/imagenes
    ...
    parser.add_argument(
        "--download-only", action="store_true", help="Solo descargar imágenes sin procesar"
    )
    parser.add_argument(
        "--benchmark", action="store_true", help="Ejecutar benchmark detallado (5 iteraciones)"
    )
    parser.add_argument(
        "--images-dir",
        type=Path,
    ...
    parser.add_argument(
        "--force-download", action="store_true", help="Re-descargar imágenes aunque ya existan"
    )
