...
def demo_basic_finetuning():
    ...
    print(f"Compressed:  {compressed_acc:6.2f}%  ({compressed_params:>9,} params, {compression_ratio:.1f}x)")
    print(f"Fine-tuned:  {tuned_acc:6.2f}%  ({compressed_params:>9,} params, {compression_ratio:.1f}x)")
...
def demo_complete_pipeline():
    ...
    print("="*70)
    print(f"Original:  {baseline_params:>8,} params  {baseline_acc:>6.2f}%  {time_baseline*10:>6.2f}ms")
    print(f"Optimized: {compressed_params:>8,} params  {tuned_acc:>6.2f}%  {time_tuned*10:>6.2f}ms")
    print(f"Savings:   {compression_ratio:>7.1f}x smaller  {baseline_acc-tuned_acc:>5.2f}% drop  {speedup:>5.2f}x faster")
    print("\n🚀 Ready to deploy!")
