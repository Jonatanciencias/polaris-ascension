...
def demo_basic_scheduling():
    ...
    scheduler = HybridScheduler(
        transfer_threshold=50 * 1024,  # 50 KB
    ...
    task_tiny = TaskConfig(
        operation=OpType.ELEMENTWISE,
        input_shapes=[(10, 10)],
        dtype='float32'
    )
    ...
    task_large = TaskConfig(
        ...
        input_shapes=[(128, 3, 224, 224), (64, 3, 7, 7)],
        dtype='float32'
    )
...
def demo_partitioning():
    ...
    if should_partition:
        ...
        t_gpu = scheduler.profiler.estimate_execution_time(task, 'cuda') * (gpu_size / batch_size) if torch.cuda.is_available() else 0
        t_transfer = scheduler.profiler.estimate_transfer_time(task.total_size * gpu_size / batch_size, 'cpu', 'cuda')
...
def demo_load_balancing():
    ...
    for i, task in enumerate(tasks, 1):
        ...
        t_gpu = scheduler.profiler.estimate_execution_time(task, 'cuda') if torch.cuda.is_available() else 0
