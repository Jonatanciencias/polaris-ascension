...
def demo_mixed_precision_basic():
    ...
    for name, sens in list(sensitivity.items())[:3]:
        print(
            f"   - {name}: sensitivity={sens.sensitivity_score:.4f}, "
    ...
    config = optimizer.search_configuration(
        x, y, method="evolutionary", population_size=10, generations=5
    )
...
def demo_neuromorphic_deployment():
    ...
    # Create simple SNN
    snn = nn.Sequential(nn.Linear(784, 256), nn.ReLU(), nn.Linear(256, 10))
...
def demo_physics_aware_quantization():
    ...
    optimizer = PhysicsAwareMixedPrecision(pinn, pde_loss_weight=1.0, physics_loss_threshold=1e-4)
...
def demo_multi_platform_export():
    ...
    # Create SNN
    snn = nn.Sequential(nn.Linear(100, 50), nn.ReLU(), nn.Linear(50, 10))
    print(f"✓ Created SNN")

    platforms = [("loihi2", "lava"), ("spinnaker2", "pynn"), ("generic", "spike_json")]
