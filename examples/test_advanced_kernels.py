...
def main():
    ...
    for size in test_sizes:
        ...
        for kernel_name, kernel in kernels.items():
            ...
            result = test_kernel(ctx, queue, kernel_name, kernel, size, A, B, C_expected, use_float4)
            
            if result:
                status = '✅' if result['success'] else '❌'
                print(f"  {result['kernel']:20s}: {result['gflops']:6.1f} GFLOPS | "
    ...
    for kernel_name in test_kernels:
        ...
        # Warmup
        for _ in range(3):
            kernel(queue, global_size, (16, 16),
                   np.int32(M), np.int32(N), np.int32(K),
                   np.float32(1.0), np.float32(0.0),
                   a_buf, b_buf, c_buf).wait()
        ...
        try:
            while time.time() - start < 15:
                ...
                evt = kernel(queue, global_size, (16, 16),
                            np.int32(M), np.int32(N), np.int32(K),
                            np.float32(1.0), np.float32(0.0),
                            a_buf, b_buf, c_buf)
                evt.wait()
