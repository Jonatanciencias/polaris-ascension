...
from distributed.load_balancing import (
    ...
    LeastLoadedBalancer, AdaptiveBalancer
)
...
class DistributedInferenceDemo:
        ...
        for task in tasks:
            ...
            try:
                ...
                self.balancer.record_task_completion(
                    worker_id,
                    result['duration'] * 1000,  # Convert to ms
                    success=True
                )
    ...
    def _show_final_statistics(self):
        ...
        print(f"  Success Rate: {len(self.completed_tasks) / max(1, len(self.completed_tasks) + len(self.failed_tasks)):.1%}")
        print()
        ...
        if self.completed_tasks:
            avg_latency = sum(t['duration'] for t in self.completed_tasks) / len(self.completed_tasks)
