...
def demo_classification_models(precision="fp32"):
    ...
    for model_info in models:
        if not model_info["path"].exists():
            ...
            print(
                f"   Download with: python scripts/download_models.py --model {model_info['path'].stem}"
            )
            continue
        ...
        try:
            ...
            print(f"\n   🎯 Results:")
            print(
                f"      Top Prediction: Class {result.get('top1_class')} ({result.get('top1_confidence', 0):.1%})"
            )
    ...
    if results:
        ...
        for r in results_sorted:
            print(
                f"{r['name']:<20} {r['inference_time_ms']:<12.1f} {r['throughput_fps']:<10.1f} {r['best_for'][:28]:<30}"
            )
...
def demo_optimization_comparison():
    ...
    precisions = [
        ...
        {"mode": "fp16", "name": "Fast (FP16)", "description": "~1.5x speedup, 73.6 dB SNR"},
        {
            ...
            "description": "~2.5x speedup, 99.99% correlation",
        },
    ]
    ...
    for prec in precisions:
        ...
        print(
            f"  ✓ Top-1: Class {result.get('top1_class')} ({result.get('top1_confidence', 0):.1%})"
        )
...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["mobilenet", "resnet50", "efficientnet", "all"],
        default="all",
        help="Model to test",
    )
    parser.add_argument(
        "--precision", choices=["fp32", "fp16", "int8"], default="fp32", help="Precision mode"
    )
    parser.add_argument(
        "--compare-optimizations", action="store_true", help="Compare FP32/FP16/INT8 on same model"
    )
