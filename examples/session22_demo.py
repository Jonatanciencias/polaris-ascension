...
def demo1_pinn_sensitivity():
    ...
    print(f"  → Gradient sensitivity: x={grad_result.feature_importance['x']:.3f}, t={grad_result.feature_importance['t']:.3f}")
    print(f"  → Integrated gradients: x={ig_result.feature_importance['x']:.3f}, t={ig_result.feature_importance['t']:.3f}")
    print(f"  → SmoothGrad: x={smooth_result.feature_importance['x']:.3f}, t={smooth_result.feature_importance['t']:.3f}")
...
def demo5_gnn_comparison():
    ...
    print(f"✓ Best throughput: {best_throughput[0]} ({best_throughput[1]['throughput']:.1f} graphs/s)")
...
def main():
    ...
    try:
        ...
        print(f"  → Feature importance: x={results['demo1']['feature_importance']['x']:.3f}, t={results['demo1']['feature_importance']['t']:.3f}")
