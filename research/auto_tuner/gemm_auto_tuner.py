...
class GEMMAutoTuner:
    ...
    def __init__(
        self,
        output_dir: str = "results/auto_tuner",
        verbose: bool = True
    ):
    ...
    def _load_kernels(self) -> Dict:
        ...
        for name, config in kernel_configs.items():
            try:
                ...
                if self.verbose:
                    print(f"✓ Loaded {name}: tile_size={config['tile_size']}, "
    ...
    def define_search_space(self) -> Dict[str, List]:
        ...
        return {
            ...
            "matrix_sizes": [
                ...
                1200, 1250, 1300, 1350, 1375, 1400, 1425, 1450, 1500, 1550, 1600,
    ...
    def run_search(self) -> Dict:
        ...
        for kernel_idx, kernel_name in enumerate(kernels):
            ...
            if kernel_results:
                ...
                print(f"\n📊 {kernel_name} summary:")
                print(f"   Best:    {best_kernel.gflops:.1f} GFLOPS @ {best_kernel.matrix_size}×{best_kernel.matrix_size}")
                print(f"   Worst:   {worst_kernel.gflops:.1f} GFLOPS @ {worst_kernel.matrix_size}×{worst_kernel.matrix_size}")
        ...
        if self.best_result:
            print(f"   tile{self.best_result.tile_size} @ {self.best_result.matrix_size}×{self.best_result.matrix_size}")
