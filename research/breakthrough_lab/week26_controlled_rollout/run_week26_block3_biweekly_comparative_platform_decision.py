#!/usr/bin/env python3
"""Week 26 Block 3: second monthly comparative + formal platform decision."""
...
def _platform_recommendation(
    *,
    ...
    return {
        "policy_id": "week26-block3-platform-policy-v1",
...
def _platform_md(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 26 Block 3 - Platform Recommendation")
    lines.append("")
...
def _dashboard_md(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 26 Block 3 - Second Monthly Comparative Dashboard")
    lines.append("")
...
def _report_md(report: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 26 Block 3 - Second Monthly Comparative + Platform Decision")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Week26 Block3 monthly comparative and platform recommendation."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="research/breakthrough_lab/week26_controlled_rollout/week26_block1_monthly_continuity_20*.json",
    )
    parser.add_argument(
        "--current-block2-report-path",
        default="research/breakthrough_lab/week26_controlled_rollout/week26_block2_alert_bridge_observability_20*.json",
    )
    parser.add_argument(
        "--current-split-eval-path",
        default="research/breakthrough_lab/week26_controlled_rollout/week26_block1_monthly_continuity_split_eval_20*.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="research/breakthrough_lab/week26_controlled_rollout/WEEK20_BLOCK1_MONTHLY_CYCLE_LIVE_DEBT_MATRIX.json",
    )
    parser.add_argument(
        "--week21-block2-debt-path",
        default="research/breakthrough_lab/week26_controlled_rollout/week26_block2_alert_bridge_observability_operational_debt_*.json",
    )
    ...
    parser.add_argument(
        "--preprod-signoff-dir",
        default="research/breakthrough_lab/week26_controlled_rollout",
    )
    parser.add_argument(
        "--output-dir",
        default="research/breakthrough_lab/week26_controlled_rollout",
    )
    parser.add_argument("--output-prefix", default="week26_block3_biweekly_comparative")
    ...
    platform_json = preprod_dir / "WEEK26_BLOCK3_PLATFORM_POLICY_DECISION.json"
    platform_md = preprod_dir / "WEEK26_BLOCK3_PLATFORM_POLICY_DECISION.md"
    ...
    debt_review_json = preprod_dir / "WEEK26_BLOCK3_OPERATIONAL_DEBT_REVIEW.json"
    _write_json(
        debt_review_json,
        {
            "review_id": "week26-block3-operational-debt-review-v1",
            "timestamp_utc": datetime.now(timezone.utc).isoformat(),
    ...
    dashboard_json = output_dir / f"week26_block3_biweekly_comparative_dashboard_{stamp}.json"
    dashboard_md = output_dir / f"week26_block3_biweekly_comparative_dashboard_{stamp}.md"
    ...
    print(f"Week26 block3 JSON: {report_json}")
    print(f"Week26 block3 MD:   {report_md}")
