#!/usr/bin/env python3
"""Week 27 Block 3: second monthly comparative + formal platform decision."""
...
def _platform_recommendation(
    *,
    ...
    return {
        "policy_id": "week27-block3-platform-policy-v1",
...
def _platform_md(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 27 Block 3 - Platform Recommendation")
    lines.append("")
...
def _dashboard_md(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 27 Block 3 - Second Monthly Comparative Dashboard")
    lines.append("")
...
def _report_md(report: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Week 27 Block 3 - Second Monthly Comparative + Platform Decision")
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Week27 Block3 monthly comparative and platform recommendation."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="research/breakthrough_lab/week25_controlled_rollout/week25_block3_biweekly_comparative_20*.json",
    )
    parser.add_argument(
        "--current-block1-report-path",
        default="research/breakthrough_lab/week27_controlled_rollout/week27_block1_monthly_continuity_20*.json",
    )
    parser.add_argument(
        "--current-block2-report-path",
        default="research/breakthrough_lab/week27_controlled_rollout/week27_block2_alert_bridge_observability_20*.json",
    )
    parser.add_argument(
        "--current-split-eval-path",
        default="research/breakthrough_lab/week27_controlled_rollout/week27_block1_monthly_continuity_split_eval_20*.json",
    )
    parser.add_argument(
        "--week20-debt-review-path",
        default="research/breakthrough_lab/week25_controlled_rollout/WEEK25_BLOCK3_OPERATIONAL_DEBT_REVIEW.json",
    )
    parser.add_argument(
        "--week21-block1-debt-path",
        default="research/breakthrough_lab/week27_controlled_rollout/WEEK20_BLOCK1_MONTHLY_CYCLE_LIVE_DEBT_MATRIX.json",
    )
    parser.add_argument(
        "--week21-block2-debt-path",
        default="research/breakthrough_lab/week27_controlled_rollout/week27_block2_alert_bridge_observability_operational_debt_*.json",
    )
    ...
    parser.add_argument(
        "--preprod-signoff-dir",
        default="research/breakthrough_lab/week27_controlled_rollout",
    )
    parser.add_argument(
        "--output-dir",
        default="research/breakthrough_lab/week27_controlled_rollout",
    )
    parser.add_argument("--output-prefix", default="week27_block3_biweekly_comparative")
    args = parser.parse_args()

    stable_manifest_path = _resolve(args.stable_manifest_path)
    baseline_comp_path = (
        _latest_glob(args.baseline_comparative_report_path)
        if "*" in args.baseline_comparative_report_path
        else _resolve(args.baseline_comparative_report_path)
    )
    current_block1_path = (
        _latest_glob(args.current_block1_report_path)
        if "*" in args.current_block1_report_path
        else _resolve(args.current_block1_report_path)
    )
    current_block2_path = (
    ...
    split_eval_path = (
        _latest_glob(args.current_split_eval_path)
        if "*" in args.current_split_eval_path
        else _resolve(args.current_split_eval_path)
    )
    week20_debt_path = (
        _latest_glob(args.week20_debt_review_path)
        if "*" in args.week20_debt_review_path
        else _resolve(args.week20_debt_review_path)
    )
    week21_block1_debt_path = (
        _latest_glob(args.week21_block1_debt_path)
        if "*" in args.week21_block1_debt_path
        else _resolve(args.week21_block1_debt_path)
    )
    ...
    baseline_comp = _read_json(baseline_comp_path) if baseline_comp_path and baseline_comp_path.exists() else {}
    current_block1 = _read_json(current_block1_path) if current_block1_path and current_block1_path.exists() else {}
    current_block2 = _read_json(current_block2_path) if current_block2_path and current_block2_path.exists() else {}
    split_eval = _read_json(split_eval_path) if split_eval_path and split_eval_path.exists() else {}
    ...
    platform_json = preprod_dir / "WEEK27_BLOCK3_PLATFORM_POLICY_DECISION.json"
    platform_md = preprod_dir / "WEEK27_BLOCK3_PLATFORM_POLICY_DECISION.md"
    ...
    debt_review_json = preprod_dir / "WEEK27_BLOCK3_OPERATIONAL_DEBT_REVIEW.json"
    _write_json(
        debt_review_json,
        {
            "review_id": "week27-block3-operational-debt-review-v1",
            "timestamp_utc": datetime.now(timezone.utc).isoformat(),
    ...
    dashboard_json = output_dir / f"week27_block3_biweekly_comparative_dashboard_{stamp}.json"
    dashboard_md = output_dir / f"week27_block3_biweekly_comparative_dashboard_{stamp}.md"
    ...
    print(f"Week27 block3 JSON: {report_json}")
    print(f"Week27 block3 MD:   {report_md}")
