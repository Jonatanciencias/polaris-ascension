...
def run_comprehensive_test():
    ...
    for name, A, B in test_matrices:
        ...
        print(f"  {status} {name:35} → {result.technique.value:15} "
    ...
    print(f"   Selecciones de alto rendimiento: {high_perf_count}/{total} ({high_perf_rate*100:.1f}%)")
    print(f"   Selecciones con alta confianza (>=80%): {high_confidence_count}/{total} ({high_conf_rate*100:.1f}%)")
    ...
    for mtype, mresults in matrix_types.items():
        if mresults:
            ...
            print(f"   {mtype}: {len(mresults)} tests, {hp_rate:.0f}% high-perf, {avg_conf:.0f}% avg conf")
    ...
    print(f"              Resultado: {high_perf_rate*100:.1f}% {'✅ PASS' if obj1_pass else '❌ FAIL'}")
    
    print(f"\n   Objetivo 2: Confianza promedio >= 80%")
    print(f"              Resultado: {avg_confidence*100:.1f}% {'✅ PASS' if obj2_pass else '❌ FAIL'}")
...
def test_edge_cases():
    ...
    A[np.random.choice(512*512, size=512*512//100, replace=False) // 512,
      np.random.choice(512*512, size=512*512//100, replace=False) % 512] = np.random.randn(512*512//100)
    result = selector.select_technique(A)
    print(f"   Técnica: {result.technique.value}, Confianza: {result.confidence:.2f}")
