...
pytestmark = pytest.mark.skipif(not OPENCL_AVAILABLE, reason="OpenCL not available")
...
class TestGEMMCorrectness:
    ...
    def test_basic_multiplication(self, cl_context):
        ...
        assert np.allclose(
            C_opencl, C_numpy, rtol=1e-4, atol=1e-5
    ...
    def test_square_matrices(self, cl_context):
        ...
        for size in [16, 32, 64, 128]:
            ...
            assert np.allclose(C_opencl, C_numpy, rtol=1e-4, atol=1e-5), f"Failed for size {size}"
...
class TestGEMMKernelVariants:
    ...
    def test_kernel_consistency(self, cl_context):
        ...
        # All should match
        assert np.allclose(C_naive, C_tiled, rtol=1e-4, atol=1e-5), "Naive and tiled kernels differ"
        assert np.allclose(C_tiled, C_2x2, rtol=1e-4, atol=1e-5), "Tiled and 2x2 kernels differ"
...
class TestGEMMPerformance:
    ...
    def test_benchmark_small(self, cl_context):
        ...
        assert results["gflops"] > 50, f"Performance too low: {results['gflops']:.2f} GFLOPS"
    ...
    def test_benchmark_medium(self, cl_context):
        ...
        assert results["gflops"] > 100, f"Performance too low: {results['gflops']:.2f} GFLOPS"
    ...
    def test_benchmark_large(self, cl_context):
        ...
        assert results["gflops"] > 200, f"Performance too low: {results['gflops']:.2f} GFLOPS"
