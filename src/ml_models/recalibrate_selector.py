...
def recalibrate_selector_with_hardware_data(benchmark_data_path: Path):
    ...
    valid_results = results_df[
        results_df["execution_time"].notna() & (results_df["execution_time"] != "inf")
    ]
    ...
    technique_performance = (
        valid_results.groupby("technique")
        .agg({"gflops": ["mean", "std", "count"], "execution_time": ["mean", "std"]})
        .round(3)
    )
    ...
    weights_data = {
        ...
        "calibration_date": (
            data["benchmark_results"][0]["timestamp"] if data["benchmark_results"] else None
        ),
