...
class CalibratedIntelligentSelector:
    ...
    def _load_hardware_calibration(self, 
    ...
    def _compute_technique_scores(self, 
                                  chars: MatrixCharacteristics) -> Dict[OptimizationTechnique, float]:
        """
        ...
        for technique, base_weight in self.technique_weights.items():
            ...
            if chars.size >= 1024:
                ...
                if technique in [OptimizationTechnique.OPENCL_GEMM, 
            ...
            if chars.matrix_type == "symmetric" and chars.is_symmetric:
                if technique in [OptimizationTechnique.QUANTUM, 
                                OptimizationTechnique.AI_PREDICTOR]:
                    score *= 1.05
    ...
    def _compute_confidence(self,
                           ...
                           chars: MatrixCharacteristics) -> float:
        """
    ...
    def select_technique(self,
                        matrix_a: np.ndarray,
                        matrix_b: Optional[np.ndarray] = None) -> SelectionResult:
        """
        ...
        if (self.prefer_ai_predictor and 
            confidence >= self.MEDIUM_CONFIDENCE_THRESHOLD and
            scores[OptimizationTechnique.AI_PREDICTOR] > 0.7):
            # AI Predictor como meta-selector
            if best_technique not in [OptimizationTechnique.AI_PREDICTOR,
                                      OptimizationTechnique.OPENCL_GEMM,
                                      OptimizationTechnique.QUANTUM]:
    ...
    def _generate_reasoning(self,
                           ...
                           confidence: float,
                           scores: Dict[OptimizationTechnique, float]) -> str:
...
def run_calibration_validation():
    ...
    print(f"   Average confidence: {stats['average_confidence']:.2f} ({stats['average_confidence']*100:.1f}%)")
    print(f"   High confidence rate: {stats['high_confidence_rate']:.2f} ({stats['high_confidence_rate']*100:.1f}%)")
    print(f"   High performance selection rate: {stats['high_performance_selection_rate']:.2f} ({stats['high_performance_selection_rate']*100:.1f}%)")
    ...
    print(f"   High-perf selection >= 90%: {high_perf_rate*100:.1f}% {'✅' if selection_ok else '❌'}")
    print(f"   Average confidence >= 80%: {avg_confidence*100:.1f}% {'✅' if confidence_ok else '❌'}")
