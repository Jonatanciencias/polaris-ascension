...
class PolarisOptimizationBenchmark:
    ...
    def benchmark_implementation(
        self, engine, engine_name: str, A: np.ndarray, B: np.ndarray, label: str
    ) -> Dict:
        ...
        try:
                ...
                result.update(
                    {
                        ...
                        "lds_utilization": (
                            getattr(metrics, "lds_utilization", 0) if "metrics" in locals() else 0
                        ),
                        "zero_copy_used": (
                            getattr(metrics.transfer_metrics, "zero_copy_used", False)
                            if "metrics" in locals() and hasattr(metrics, "transfer_metrics")
                            else False
                        ),
                    }
                )
            ...
            return {"engine": engine_name, "matrix_size": label, "success": False, "error": str(e)}
    ...
    def generate_performance_report(self, df: pd.DataFrame) -> str:
        ...
        for matrix_size in df["matrix_size"].unique():
            ...
            for _, row in matrix_data.iterrows():
                report.append(
                    f"  {row['engine']}: {row['avg_gflops']:.1f} GFLOPS (peak: {row['peak_gflops']:.1f})"
                )
        ...
        if len(df["engine"].unique()) >= 2:
            ...
            for engine in engines_list[1:]:
                ...
                avg_improvement = (
                    engine_data["avg_gflops"].mean() / baseline_data["avg_gflops"].mean() - 1
                ) * 100
                peak_improvement = (
                    engine_data["peak_gflops"].max() / baseline_data["peak_gflops"].max() - 1
                ) * 100
    ...
    def create_visualizations(
        self, df: pd.DataFrame, save_path: str = "polaris_benchmark_results.png"
    ):
        ...
        try:
            ...
            fig.suptitle(
                "🚀 Polaris Breakthrough Optimization Benchmark Results",
                fontsize=16,
                fontweight="bold",
            )
            ...
            for i, engine in enumerate(engines):
                ...
                performance = [
                    (
                        engine_data[engine_data["matrix_size"] == size]["avg_gflops"].iloc[0]
                        if not engine_data[engine_data["matrix_size"] == size].empty
                        else 0
                    )
            ...
            if len(engines) > 1:
                ...
                baseline_perf = df[df["engine"] == baseline_engine].set_index("matrix_size")[
                    "avg_gflops"
                ]
                ...
                for engine in engines[1:]:
                    ...
                    ax2.bar(
                        ...
                        improvement,
                        label=f"{engine} vs {baseline_engine}",
                        alpha=0.8,
                    )
            ...
            for engine in engines:
                ...
                bandwidth = [
                    (
                        engine_data[engine_data["matrix_size"] == size]["bandwidth_gbs"].iloc[0]
                        if not engine_data[engine_data["matrix_size"] == size].empty
                        else 0
                    )
                    for size in matrix_sizes
